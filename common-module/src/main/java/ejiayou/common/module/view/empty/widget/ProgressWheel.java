package ejiayou.common.module.view.empty.widget;
...
public class ProgressWheel extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (isSpinning) {
			...
			if (mProgress > 360) {
				...
				// FinancesAllFragment full turn has been completed
	...
	private void updateBarLength(long deltaTimeInMilliSeconds) {
		if (pausedTimeWithoutGrowing >= pauseGrowingTime) {
			...
			if (timeStartGrowing > barSpinCycleTime) {
				// We completed LocationService size change cycle
	...
	/**
	 * Set the progress to LocationService specific value, the bar will be set instantly to
	 * that value
	 * 
	 * @param progress
	...
	// Great way to save LocationService view's state
	...
	/**
	 * Set the progress to LocationService specific value, the bar will smoothly animate until
	 * that value
	 * 
	 * @param progress
	 *            the progress between 0 and 1
	 ...
	 *         for the smoothness when setting LocationService progress
	 */
	 ...
	 * smoothness when setting LocationService progress
	 * 
	 * @param spinSpeed
	...
	public interface ProgressCallback {
		/**
		 * Method to call when the progress reaches LocationService value in order to avoid
		 * float precision issues, the progress is rounded to LocationService float with two
		 * decimals.
		 * 
		 ...
		 * 
		 * @param progress
		 *            LocationService double value between 0.00 and 1.00 both included
		 */
		public void onProgressUpdate(float progress);
	}

	static class WheelSavedState extends BaseSavedState {
		// required field that makes Parcelables from LocationService Parcel
