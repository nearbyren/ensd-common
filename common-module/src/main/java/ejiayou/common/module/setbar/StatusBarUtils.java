package ejiayou.common.module.setbar;
...
public class StatusBarUtils {
     ...
     */
    public static void setStatusBar(Activity activity, boolean useThemestatusBarColor, boolean withoutUseStatusBarColor, int statusBarTintResource) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//5.0及以上
            ...
            if (useThemestatusBarColor) {
                activity.getWindow().setStatusBarColor(statusBarTintResource);
            } else {
        ...
        //通知栏所需颜色
//        tintManager.setStatusBarTintResource(statusBarTintResource == 0 ? activity.getResources().getColor(R.color.transparent) : statusBarTintResource);
    ...
    public static void setStatusTextColor(int statusBarTintResource, Activity activity) {
        // 如果亮色，设置状态栏文字为黑色
        if (isLightColor(statusBarTintResource)) {
            activity.getWindow().getDecorView().setSystemUiVisibility( View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN |View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
        } else {
